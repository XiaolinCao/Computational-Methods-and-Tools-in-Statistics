...
##b
#### function to compute the angle ?? formed by the origin 
...
compute_theta = function(x) {
  ...
  return(theta)
  # if (x[1] == 0) {
  #   # split special when x = 0
  #   return( sign(x[2]) * pi / 2)
  # } else if (x[1] > 0) {
  #   # x > 0
  #   return( sign(x[2]) * atan( abs(x[2] / x[1])))
  # } else {
  #   # x < 0
  #   return( sign(x[2]) * {pi - atan( abs(x[2] / x[1]))})
  # }
}
...
train_dat = read.csv("/Users/caoxiaolin/Downloads/STATS506/hw2/train_trajectories.csv")
...
#### results in a table
test_dat = read.csv("/Users/caoxiaolin/Downloads/STATS506/hw2/test_trajectories.csv")
